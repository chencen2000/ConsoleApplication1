...
namespace ConsoleApplication1
{
    class opencv
    {
        ...
        static void test()
        {
            ...
            string dir1 = @"C:\Users\qa\Desktop\picture\iphone_icon\phone_icons";
            MCvTermCriteria term = new MCvTermCriteria(100, 0.00001d);
            BOWKMeansTrainer bowtrainer = new BOWKMeansTrainer(clusters, term, 3, Emgu.CV.CvEnum.KMeansInitType.PPCenters);
            Mat mm = new Mat();
            int count = 0;
            foreach (string s in System.IO.Directory.GetFiles(dir1))
            {
                Mat m = CvInvoke.Imread(s, Emgu.CV.CvEnum.ImreadModes.Grayscale);
            ...
            foreach (string s in System.IO.Directory.GetFiles(dir1))
            {
                ...
                if (string.Compare(System.IO.Path.GetFileNameWithoutExtension(s), "icon_19") == 0 ||
                    string.Compare(System.IO.Path.GetFileNameWithoutExtension(s), "temp_5x3") == 0)
                    label[count, 0] = 1;
                else label[count, 0] = -1;
                count++;
            }
            ...
            TrainData td = new TrainData(train_data, Emgu.CV.ML.MlEnum.DataLayoutType.RowSample, label);
            bool tained = svm.TrainAuto(td);
            //bool td = svm.Train(train_data, Emgu.CV.ML.MlEnum.DataLayoutType.RowSample, label);

            // test
            {
                Mat img = CvInvoke.Imread(@"C:\Users\qa\Desktop\picture\iphone_icon\setting_icon.jpg", Emgu.CV.CvEnum.ImreadModes.Grayscale);
