...
namespace ConsoleApplication1
{
    class ColorSensor
    {
        ...
        static string parse_color_data(string line)
        {
            StringBuilder sb = new StringBuilder();
            if (!string.IsNullOrEmpty(line))
            {
                Regex reg = new Regex(@"^Color Temp: (\d+) K - Lux: (\d+) - R: (\d+) G: (\d+) B: (\d+) C: (\d+)\s*$");
                // input line=Color Temp: 3430 K - Lux: 779 - R: 1739 G: 1283 B: 931 C: 3980  
                // output=3476,645,1602,1145,877,3647,112,80,61
        ...
        static private void test_data(out Matrix<float> data, string line="")
        {
            /*
            ...
            string raw_data = string.IsNullOrEmpty(line) ? "3476,645,1602,1145,877,3647,112,80,61" : line;
            int varCount = raw_data.Split(',').Length;
            data = new Matrix<float>(1, varCount);
            string[] values = raw_data.Split(',');
            for (int i = 0; i < values.Length; i++)
